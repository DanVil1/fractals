...
import { Play, Pause, Maximize, Volume2, Download, Moon, Sun, ArrowLeft, Layers, Activity, Wind, Sprout, Aperture, Flower, Infinity, GitBranch, Shapes, Triangle, Rotate3D, CloudFog, Zap, Snowflake, Eye, Hourglass } from 'lucide-react';
...
const rodriguesRotate = (v, k, theta) => {
...
const FlowerOfLife = ({ layers, zoom, isDark }) => {
  const radius = 30;
  const generateCircles = () => {
    ...
    if (layers >= 1) {
      for (let i = 0; i < 6; i++) {
        ...
        results.push({ cx: Math.cos(angle) * radius, cy: Math.sin(angle) * radius, key: `l1-${i}` });
      }
    }
    if (layers >= 2) {
      for (let ring = 2; ring <= layers; ring++) {
        for (let i = 0; i < 6; i++) {
            const angle = (i * 60) * (Math.PI / 180);
            results.push({ cx: Math.cos(angle) * radius * ring, cy: Math.sin(angle) * radius * ring, key: `r${ring}-tip-${i}` });
            for(let j = 1; j < ring; j++) {
                 ...
                 results.push({ cx: startX + (endX - startX) * ratio, cy: startY + (endY - startY) * ratio, key: `r${ring}-edge-${i}-${j}` });
            }
        }
      }
    }
  ...
  return (
    <div className="w-full h-full flex items-center justify-center overflow-hidden cursor-move">
      <svg viewBox="-200 -200 400 400" className="w-full h-full transition-transform duration-100 ease-linear" style={{ transform: `scale(${zoom})` }}>
        <g>
          {data.map((c) => (
            <circle key={c.key} cx={c.cx} cy={c.cy} r={radius} fill="none" stroke={isDark ? "cyan" : "#e11d48"} strokeWidth="1.5" className="opacity-60 mix-blend-screen transition-all duration-500" />
          ))}
           <circle cx={0} cy={0} r={radius * (layers + 1)} fill="none" stroke={isDark ? "white" : "black"} strokeWidth="2" strokeDasharray="5,5" className="opacity-30" />
        </g>
      </svg>
    </div>
  );
...
const BronchialTree = ({ depth, isBreathing, isDark }) => {
  ...
  useAnimationFrame((deltaTime) => {
    ...
    if (isBreathing) setScale(1 + Math.sin(timeRef.current * 2) * 0.1);
    else setScale(1);
    drawTree();
  });
  const drawTree = () => {
    ...
    const drawBranch = (x, y, z, len, angleX, angleZ, branchWidth, level) => {
      const p1 = project(x, y, z);
      const endX = x + Math.sin(angleZ) * Math.cos(angleX) * len * scale;
      const endY = y + Math.cos(angleZ) * len * scale;
      const endZ = z + Math.sin(angleZ) * Math.sin(angleX) * len * scale;
  ...
  useEffect(() => { const c = canvasRef.current; if(c){ c.width = c.parentElement.clientWidth; c.height = c.parentElement.clientHeight;} }, []);
  return <canvas ref={canvasRef} className="w-full h-full" />;
};

const ChladniPlate = ({ frequency, isDark }) => {
  const canvasRef = useRef(null);
  const particlesRef = useRef([]);
  const animationRef = useRef(null);
  useEffect(() => { particlesRef.current = Array.from({ length: 3000 }, () => ({ x: Math.random(), y: Math.random() })); }, []);
  const getChladniValue = (x, y, n, m) => Math.cos(n * Math.PI * x) * Math.cos(m * Math.PI * y) - Math.cos(m * Math.PI * x) * Math.cos(n * Math.PI * y);
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
  ...
  return (
    <div className="w-full h-full relative">
        ...
        <div className="absolute top-4 right-4 font-mono text-xs bg-black/20 p-1 rounded">n={1 + Math.floor(frequency / 100)}, m={2 + Math.floor(frequency / 150)}</div>
    </div>
  );
};

const BarnsleyFern = ({ isDark, density = 1 }) => {
    const canvasRef = useRef(null);
    const animationRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const renderBatch = () => {
             const w = canvas.width; const h = canvas.height; const scale = Math.min(w, h) / 12;
             ctx.fillStyle = isDark ? '#4ade80' : '#15803d';
             const pointsPerFrame = 50 * density;
             for(let i=0; i<pointsPerFrame; i++) {
                 let { x, y } = pointRef.current; let nextX, nextY; const r = Math.random();
                 ...
                 const plotX = w/2 + nextX * scale; const plotY = h - nextY * scale;
...
const Phyllotaxis = ({ isDark, spacing = 4, size = 2 }) => {
    const canvasRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const render = () => {
             ...
             const count = 1500; const goldenAngle = 137.5 * (Math.PI / 180);
             for (let n = 0; n < count; n++) {
                 const r = spacing * Math.sqrt(n); const theta = n * goldenAngle;
...
const MaurerRose = ({ isDark, n = 6, d = 71 }) => {
    const canvasRef = useRef(null);
    useEffect(() => {
        ...
        ctx.strokeStyle = isDark ? 'rgba(236, 72, 153, 0.6)' : 'rgba(219, 39, 119, 0.6)'; ctx.lineWidth = 0.5; ctx.beginPath();
        for (let i = 0; i <= 360; i++) {
            const k = i * d * Math.PI / 180; const r = Math.sin(n * k) * scale;
            const x = r * Math.cos(k); const y = r * Math.sin(k);
            if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
        }
        ctx.stroke();
        ctx.strokeStyle = isDark ? '#fff' : '#000'; ctx.lineWidth = 2; ctx.beginPath();
        for (let i = 0; i <= 360; i++) {
            const k = i * Math.PI / 180; const r = Math.sin(n * k) * scale;
            const x = r * Math.cos(k); const y = r * Math.sin(k);
            if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
        }
        ctx.stroke();
...
const LorenzAttractor = ({ isDark, speed = 1 }) => {
    const canvasRef = useRef(null);
    const animationRef = useRef(null);
    const posRef = useRef({ x: 0.1, y: 0, z: 0 });
    const pointsRef = useRef([]); 
    useEffect(() => { posRef.current = { x: 0.1, y: 0, z: 0 }; pointsRef.current = []; }, []);
    useEffect(() => {
        const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d');
        const sigma = 10; const rho = 28; const beta = 8/3;
        const render = () => {
             if (canvas.width !== canvas.parentElement.clientWidth) { canvas.width = canvas.parentElement.clientWidth; canvas.height = canvas.parentElement.clientHeight; }
             const w = canvas.width; const h = canvas.height; const scale = 8;
             const dt = 0.01 * speed; let { x, y, z } = posRef.current;
             for(let i=0; i<5; i++) {
                 const dx = (sigma * (y - x)) * dt; const dy = (x * (rho - z) - y) * dt; const dz = (x * y - beta * z) * dt;
                 x += dx; y += dy; z += dz;
                 pointsRef.current.push({ x, y, z });
             }
             posRef.current = { x, y, z };
             if (pointsRef.current.length > 2000) pointsRef.current.splice(0, 10);
             ctx.clearRect(0, 0, w, h); ctx.save(); ctx.translate(w/2, h/2);
             ctx.beginPath();
             pointsRef.current.forEach((p, i) => {
                 if (i === 0) ctx.moveTo(p.x * scale, -p.z * scale + 200); else ctx.lineTo(p.x * scale, -p.z * scale + 200);
             });
             ctx.strokeStyle = isDark ? '#f472b6' : '#db2777'; ctx.lineWidth = 1.5; ctx.stroke(); ctx.restore();
             animationRef.current = requestAnimationFrame(render);
        };
...
const LSystemTree = ({ isDark, iterations, angleDeg }) => {
    const canvasRef = useRef(null);
    useEffect(() => {
        const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d');
        canvas.width = canvas.parentElement.clientWidth; canvas.height = canvas.parentElement.clientHeight;
        const w = canvas.width; const h = canvas.height;
        const axiom = "F";
        const rule = { "F": "FF+[+F-F-F]-[-F+F+F]" };
        let sentence = axiom;
        for (let i = 0; i < iterations; i++) {
            let nextSentence = "";
            for (let char of sentence) { if (rule[char]) nextSentence += rule[char]; else nextSentence += char; }
            sentence = nextSentence;
        }
        ctx.clearRect(0, 0, w, h); ctx.strokeStyle = isDark ? '#a7f3d0' : '#064e3b';
        const len = (h / 4) / (Math.pow(2, iterations) * 0.5 + 1); const rad = angleDeg * Math.PI / 180;
        ctx.translate(w/2, h); ctx.lineWidth = 1;
        const stack = [];
        for (let char of sentence) {
            if (char === "F") { ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(0, -len); ctx.stroke(); ctx.translate(0, -len); } 
            else if (char === "+") { ctx.rotate(rad); } 
            else if (char === "-") { ctx.rotate(-rad); } 
            else if (char === "[") { stack.push({ x: 0, y: 0, transform: ctx.getTransform() }); } 
            else if (char === "]") { const state = stack.pop(); if(state) ctx.setTransform(state.transform); }
        }
    }, [isDark, iterations, angleDeg]);
    return <canvas ref={canvasRef} className="w-full h-full" />;
};

const WindyPlant = ({ isDark, iterations, angleDeg }) => {
    const canvasRef = useRef(null);
    const particlesRef = useRef([]);
    const treeDataRef = useRef({ lines: [], spawns: [], minX: 0, minY: 0, width: 0, height: 0 });
    ...
    useEffect(() => {
        ...
        for (let char of sentence) {
            ...
            else if (char === "+") { angle += rad; } 
            else if (char === "-") { angle -= rad; } 
            else if (char === "[") { stack.push({ x: cx, y: cy, a: angle }); } 
            else if (char === "]") { const s = stack.pop(); if(s) { cx = s.x; cy = s.y; angle = s.a; } }
        }

        treeDataRef.current = { lines, spawns, width: maxX - minX, height: maxY - minY, minX, minY };
    ...
    useAnimationFrame(() => {
        ...
        ctx.strokeStyle = isDark ? '#bef264' : '#65a30d'; ctx.lineWidth = 1;
        ctx.beginPath();
        for(let l of lines) { ctx.moveTo(l.x1 * scale + offsetX, l.y1 * scale + offsetY); ctx.lineTo(l.x2 * scale + offsetX, l.y2 * scale + offsetY); }
        ctx.stroke();

        if (spawns.length > 0 && Math.random() < 0.2) { 
             const spawn = spawns[Math.floor(Math.random() * spawns.length)];
             particlesRef.current.push({ x: spawn.x * scale + offsetX, y: spawn.y * scale + offsetY, vx: (Math.random() - 0.2) * 2, vy: (Math.random() - 0.5) * 2, life: 1.0, rotation: Math.random() * Math.PI });
        }
        ...
        for (let i = particlesRef.current.length - 1; i >= 0; i--) {
            ...
            p.vx += windForceX + (Math.random() - 0.5) * 0.05; p.vy += windForceY; p.x += p.vx; p.y += p.vy; p.life -= 0.005; p.rotation += 0.1;
            if (p.life > 0 && p.x > -50 && p.x < w + 50 && p.y < h + 50) {
                ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(p.rotation); ctx.beginPath(); ctx.ellipse(0, 0, 3, 1.5, 0, 0, Math.PI * 2); ctx.fill(); ctx.restore();
...
const SuperformulaShape = ({ isDark, m, n1, n2, n3 }) => {
    const canvasRef = useRef(null);
    const animationRef = useRef(null);
    const timeRef = useRef(0);
    useEffect(() => {
        ...
        const render = () => {
             ...
             ctx.clearRect(0, 0, w, h); ctx.save(); ctx.translate(w/2, h/2);
             const scale = Math.min(w, h) / 2.5;
             ctx.strokeStyle = isDark ? '#38bdf8' : '#0284c7'; ctx.lineWidth = 2;
             timeRef.current += 0.005; ctx.rotate(timeRef.current);
             ctx.beginPath();
             const a = 1; const b = 1; const totalPoints = 360 * 2;
             for (let i = 0; i <= totalPoints; i++) {
                 ...
                 if (isFinite(r)) { const x = r * Math.cos(phi) * scale; const y = r * Math.sin(phi) * scale; if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); }
             }
             ctx.closePath(); ctx.stroke();
             ctx.fillStyle = isDark ? 'rgba(56, 189, 248, 0.1)' : 'rgba(2, 132, 199, 0.1)'; ctx.fill(); ctx.restore();
             animationRef.current = requestAnimationFrame(render);
        };
        render(); return () => cancelAnimationFrame(animationRef.current);
    }, [isDark, m, n1, n2, n3]);
...
const SierpinskiTetrahedron = ({ isDark, depth, axisX, axisY, axisZ }) => {
    const canvasRef = useRef(null);
    ...
    useEffect(() => {
        const canvas = canvasRef.current; if(!canvas) return; const ctx = canvas.getContext('2d');
        const r = 200;
        const initialVertices = [ { x: r, y: r, z: r }, { x: -r, y: -r, z: r }, { x: -r, y: r, z: -r }, { x: r, y: -r, z: -r } ];
        const drawTetra = (vertices) => {
             const project = (v) => {
                 const fov = 400; const distance = 500;
                 let len = Math.sqrt(axisX*axisX + axisY*axisY + axisZ*axisZ); if(len === 0) len = 1;
             ...
             ctx.beginPath();
             edges.forEach(([i, j]) => { ctx.moveTo(p2d[i].x, p2d[i].y); ctx.lineTo(p2d[j].x, p2d[j].y); });
             ctx.stroke();
        };
        const generate = (v, d) => {
             if (d === 0) { drawTetra(v); return; }
             const mid = (p1, p2) => ({ x: (p1.x + p2.x)/2, y: (p1.y + p2.y)/2, z: (p1.z + p2.z)/2 });
             const m01 = mid(v[0], v[1]); const m02 = mid(v[0], v[2]); const m03 = mid(v[0], v[3]);
             const m12 = mid(v[1], v[2]); const m13 = mid(v[1], v[3]); const m23 = mid(v[2], v[3]);
             generate([v[0], m01, m02, m03], d - 1); generate([m01, v[1], m12, m13], d - 1);
             generate([m02, m12, v[2], m23], d - 1); generate([m03, m13, m23, v[3]], d - 1);
        };
        const render = () => {
             if (canvas.width !== canvas.parentElement.clientWidth) { canvas.width = canvas.parentElement.clientWidth; canvas.height = canvas.parentElement.clientHeight; }
             const w = canvas.width; const h = canvas.height;
             ctx.clearRect(0,0, w, h); ctx.strokeStyle = isDark ? '#a855f7' : '#7e22ce'; ctx.lineWidth = 1;
...
const DragonCurve = ({ isDark, iterations }) => {
    const canvasRef = useRef(null);
    useEffect(() => {
        const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d');
        canvas.width = canvas.parentElement.clientWidth; canvas.height = canvas.parentElement.clientHeight;
        ...
        for (let i = 0; i < iterations; i++) {
            let nextSentence = "";
            for (let char of sentence) { if (rules[char]) nextSentence += rules[char]; else nextSentence += char; }
            sentence = nextSentence;
        }
        ...
        for (let char of sentence) {
            if (char === "F") {
                cx += Math.cos(angle) * unit; cy += Math.sin(angle) * unit;
                minX = Math.min(minX, cx); maxX = Math.max(maxX, cx);
                minY = Math.min(minY, cy); maxY = Math.max(maxY, cy);
            } 
        ...
        const treeW = maxX - minX; const treeH = maxY - minY;
        const padding = 40;
        ...
        for (let char of sentence) {
            if (char === "F") { ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(scale, 0); ctx.stroke(); ctx.translate(scale, 0); } 
            else if (char === "+") { ctx.rotate(Math.PI / 2); } 
            else if (char === "-") { ctx.rotate(-Math.PI / 2); }
        }
    }, [isDark, iterations]);
    return <canvas ref={canvasRef} className="w-full h-full" />;
...
const KochSnowflake = ({ isDark, iterations }) => {
    const canvasRef = useRef(null);
    useEffect(() => {
        const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d');
        canvas.width = canvas.parentElement.clientWidth; canvas.height = canvas.parentElement.clientHeight;
        ...
        ctx.clearRect(0, 0, w, h);
        ctx.strokeStyle = isDark ? '#3b82f6' : '#2563eb'; // Blue
        ctx.lineWidth = 1.5;
        ctx.translate(offsetX, offsetY);
        
        for (let char of sentence) {
            if (char === "F") {
                ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(scale, 0); ctx.stroke(); ctx.translate(scale, 0);
            } 
            else if (char === "+") { ctx.rotate(rad); } 
            else if (char === "-") { ctx.rotate(-rad); }
        }
    ...
    useEffect(() => {
        ...
        for (let y = 0; y < renderH; y++) {
            for (let x = 0; x < renderW; x++) {
                ...
                if (i === maxIter) {
                    data[pIndex] = 0; data[pIndex + 1] = 0; data[pIndex + 2] = 0; data[pIndex + 3] = 255;
                } else {
                    const t = i / maxIter;
                    ...
                    if (isDark) {
                        data[pIndex] = r * 2 + 50; data[pIndex + 1] = g * 4 + 20; data[pIndex + 2] = b * 10 + 100;
                    } else {
                        data[pIndex] = r * 3; data[pIndex + 1] = g * 3; data[pIndex + 2] = b * 6 + 50;
                    }
                    data[pIndex + 3] = 255;
                }
            }
...
// --- UPDATED COMPONENT: MATHEMATICAL LIFECYCLE FLOWER (FIXED PHYSICS) ---
const LifecycleFlower = ({ isDark, speed = 1 }) => {
    const canvasRef = useRef(null);
    const timeRef = useRef(0);
    const noiseSeed = useRef(Array.from({length: 100}, () => Math.random()));
    ...
    useAnimationFrame((deltaTime) => {
        ...
        ctx.strokeStyle = isDark ? `rgba(163, 230, 53, ${decay})` : `rgba(101, 163, 13, ${decay})`;
        ctx.lineWidth = 2;
        ctx.beginPath();
        const stemBase = h;
        const stemMaxHeight = h - (cy - 150); // Total potential height
        const currentStemHeight = stemMaxHeight * stemGrowth; // Current physical height
        ...
        for (let i = 0; i <= stemSteps; i++) {
            const progress = i / stemSteps; // 0 to 1 along the CURRENT stem length
            
            const y = stemBase - currentStemHeight * progress;
            
            // Wither bending equation applied to the whole stem structure
            // We map 'progress' to the 'global height ratio' to keep bend consistent
        ...
        // Must match the last point of the stem loop exactly
        const tipProgress = 1; // Top of current stem
        ...
        const tipY = stemTopY; // Already calculated based on growth
        ...
        if (seedCount > 1) {
            ...
            if (petalSize > 1) {
                ctx.strokeStyle = isDark ? `rgba(244, 114, 182, ${decay})` : `rgba(219, 39, 119, ${decay})`;
                ctx.lineWidth = 1;
                ctx.beginPath();
...
export default function FractalGallery() {
  ...
  const handleExport = () => {
      ...
      if(canvas) {
          const link = document.createElement('a'); link.download = `fractal-${activeFractal}.png`; link.href = canvas.toDataURL(); link.click();
      }
  };
